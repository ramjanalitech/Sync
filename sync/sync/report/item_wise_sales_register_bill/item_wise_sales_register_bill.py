...
#
# CUSTOMIZED: Item-wise Sales Register - Bill
# Only fetches Sales Invoices where the "bill" checkbox (custom_bill) is checked.
...
# ── CHANGE 1: added `bill` filter inside apply_conditions ─────────────────────
def apply_conditions(query, si, sii, sip, filters, additional_conditions=None):
	...
	# ── Bill filter: ALWAYS hardcoded to 1 ──────────────────────────────────
	# This report is exclusively for Bill-checked invoices.
	# Hardcoding ensures the filter works even when the frontend
	# sends empty filters (are_default_filters=true, filters={}).
	query = query.where(si.custom_bill == 1)
	# ─────────────────────────────────────────────────────────────────────────
...
# ── CHANGE 2: select si.custom_bill in the query so the field is available ────
def get_items(filters, additional_query_columns, additional_conditions=None):
	...
	query = (
		...
		.select(
			...
			sii.uom,
			sii.qty,
			si.custom_bill,  # ← ADDED: fetch the bill checkbox value
		)
...
def get_tax_accounts(
	...
	# ERPNext v15: `tabItem Wise Tax Detail` no longer exists.
	# Tax per item is stored as a JSON blob in `item_wise_tax_detail`
	# on each row of `tabSales Taxes and Charges`.
	# We read all tax rows for the invoices in item_list, parse the JSON,
	# and match amounts back to invoice item rows by their `name` (item_row key).

	import json

	# Build a set of parent invoice names from item_list
	invoice_names = list(set(d.parent for d in item_list))
	# Map item row name → invoice item dict (need parent for join)
	item_row_map = {d.name: d for d in item_list}
	...
	# Fetch all tax rows for the relevant invoices
	tax_rows = (
		frappe.qb.from_(taxes_and_charges)
		.left_join(account)
		.on(taxes_and_charges.account_head == account.name)
		.select(
			taxes_and_charges.parent,
			taxes_and_charges.account_head,
			taxes_and_charges.description,
			taxes_and_charges.charge_type,
			taxes_and_charges.rate,
			taxes_and_charges.item_wise_tax_detail,
			account.account_type,
		)
		.where(taxes_and_charges.parent.isin(invoice_names))
		.where(taxes_and_charges.parenttype == doctype)
		.run(as_dict=True)
	)
	...
	for tax_row in tax_rows:
		...
		description = handle_html(tax_row.description) or tax_row.account_head
		...
		is_other_charges = 0 if tax_row.account_type == "Tax" else 1

		try:
			item_tax_map = json.loads(tax_row.item_wise_tax_detail)
		except (ValueError, TypeError):
			continue

		# item_tax_map = { item_row_name: [tax_rate, tax_amount], ... }
		for item_row_name, tax_data in item_tax_map.items():
			if item_row_name not in item_row_map:
				continue

			if not isinstance(tax_data, (list, tuple)) or len(tax_data) < 2:
				continue

			tax_rate = flt(tax_data[0])
			tax_amount = flt(tax_data[1], precision)

			if tax_amount == 0:
				continue

			# Register column
			if scrubbed_description not in tax_columns:
				tax_columns[scrubbed_description] = description

			rate = "NA" if tax_rate == 0 else tax_rate

			itemised_tax.setdefault(item_row_name, {}).setdefault(
			...
			itemised_tax[item_row_name][scrubbed_description].tax_amount += tax_amount
...
# get_tax_details_query is no longer used in ERPNext v15 (tabItem Wise Tax Detail removed).
# Kept as a no-op stub so any external callers don't break.
def get_tax_details_query(doctype, tax_doctype):
	raise NotImplementedError(
		"get_tax_details_query is not supported in ERPNext v15. "
		"tabItem Wise Tax Detail table no longer exists. "
		"Use get_tax_accounts() which reads item_wise_tax_detail JSON directly."
	)
